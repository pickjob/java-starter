 ...
 * @date: 2022-11-29
 */
public class CommonCompressShowCase {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Tar Archive && XZ Compress
        try (OutputStream fo = Files.newOutputStream(Paths.get("logs/logs.tar.xz"));
             OutputStream bo = new BufferedOutputStream(fo);
             OutputStream xzo = new XZCompressorOutputStream(bo);
             ArchiveOutputStream o = new TarArchiveOutputStream(xzo)) {
            for (Path log : logs) {
                File f = log.toFile();
                ArchiveEntry entry = o.createArchiveEntry(f, new File(f.getParent()).getName() + "-" + f.getName());
        ...
        try (InputStream fi = Files.newInputStream(Paths.get("logs/logs.tar.xz"));
            ...
            while ((entry = i.getNextEntry()) != null) {
                ...
                String name = targetDir + "/" + entry.getName();
